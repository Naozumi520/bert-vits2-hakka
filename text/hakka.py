...
def rom_to_initials_medials_finals_tones(jyutping_syllables):
    initials_medials_finals = []
    tones = []
    word2ph = []

    for syllable in jyutping_syllables:
        if syllable in punctuation:
            initials_medials_finals.append(syllable)
            tones.append(0)
            word2ph.append(1)
        else:
            try:
            ...
            for initial in INITIALS:
                if syllable_without_tone.startswith(initial):
                    ...
                    if initial == "j":
                        medial = "i"
                    elif any(final.startswith(f) for f in med_rhymes):
                        medial = "i"
                        final = final[1:]
                    else:
                        medial = "#"

                    initials_medials_finals.extend([initial, medial, final])
                    tones.extend([tone, 0 if medial == "#" else tone, tone])
                    word2ph.append(3)
                    break
            else:
                initial = syllable_without_tone
                medial = "#"
                final = initial[-1] if initial else initial

                initials_medials_finals.extend([initial, medial, final])
                tones.extend([tone, 0, tone])
                word2ph.append(3)

    print(initials_medials_finals)
    assert len(initials_medials_finals) == len(tones)
    assert sum(word2ph) == len(initials_medials_finals)
    return initials_medials_finals, tones, word2ph
...
def jyutping2hakka(j):
    ROM_MAPPING = {
        "aa": "a",
        "j": "y",
    }

    return reduce(lambda pron, rule: pron.replace(*rule), ROM_MAPPING.items(), j)
...
def g2p(text, g2p_bypass=False):
    ...
    #     rom = [jyutping2hakka(j) for j in jyuping]
    #     phones, tones, word2ph = rom_to_initials_medials_finals_tones(rom)
    # else:
    phones, tones, word2ph = rom_to_initials_medials_finals_tones(text)
    phones = ["_"] + phones + ["_"]
    tones = [0] + tones + [0]
    word2ph = [1] + word2ph + [1]
    ...
    with open(metadata, "r", encoding="utf-8") as _file_:
        if dataset == "ciugo":
            ...
            for row in tqdm.tqdm(reader, desc="Processing dataset"):
                ...
                try:
                    phones, tones, word2ph = rom_to_initials_medials_finals_tones(
                        rom_syllables
                    )
...
if __name__ == "__main__":
    ...
    if args.dataset:
        ...
        text = "gin1 ngit5 tien1 hi4 hau3"
